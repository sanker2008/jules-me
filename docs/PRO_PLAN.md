# JulesMe Pro 候选产品与技术规划文档

> **文档性质**：本文档包含尚未实现、尚未排期的候选方案，不代表当前产品能力或对用户的上线承诺。当前真实状态以 `docs/PRO_IMPLEMENTATION_STATUS.md` 和运行时代码为准；Relay、R2、Patch 分享及支付能力必须完成工程、安全、成本与合规验证后才能进入产品文案。

## 当前优先级（2026-09-07）

1. **P0：可信体验**——免费版保留完整任务闭环，连接、执行和交付状态都使用真实数据。
2. **P0：Pro 工作流**——优先打磨自定义 Prompt、全局编码规范、自动注入提示与真实交付战报。
3. **P1：可上线授权**——完成服务端授权、支付与设备解绑闭环后，再开放购买入口。
4. **P2：候选云能力**——Relay、R2 上传与 Patch 分享仅作为候选方案，不进入当前权益宣传。
...
| **图片附件（Pro 专属）** | 不支持附加；可查看历史图片 | 本地 Base64 附件已实现；R2 上传待实现 | 本地 Base64 附件已实现；R2 上传待实现 |
...
* 免费版：不开放图片附件入口与发送；保留历史图片及产物预览。
* Pro 版当前：读取本地图片，以 Base64 附件提交任务；选择和发送均检查 Pro 权益。
* Pro 版后续候选：图片上传 Cloudflare R2，Prompt 引用图片 URL；服务端必须再次校验授权、设备与上传额度，完成后才宣传高清图床或多图能力。
